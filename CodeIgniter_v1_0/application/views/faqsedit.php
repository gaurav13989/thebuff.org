...
<script type="text/javascript">
	$(function(){
		$('.edit').live('click',function(){
			var p = {};
	       	p['faqId'] = $(this).parent().parent().find('.faqId').val();
	        p['faq'] = $(this).parent().parent().find('.faq').val();
	        p['faqa'] = $(this).parent().parent().find('.faqa').val();
	        ...
	        $(this).parent().load('/v1_0/update/faqs/'+p['faqId'],p,function(html){
	        	if(html == "y")
		        {
		...
		$('.delete').live("click",function(){
			...
			if(temp)
			{
		        ...
		        $(this).parent().parent().find(".loading").html('<img src="<?=base_url();?>images/loading.gif" />');
		        $(this).parent().load('/v1_0/delete/faqs/'+p['faqId'],p,function(html){
		        	if(html == "y")
		        	{
		        		$(this).parent().find(".loading").html('<img src="<?=base_url();?>images/tick.jpg" />');
...
<div class="records" style="text-align: center; margin-top: 10px">
	<table>
		<tr>
			<th width="100px">FAQID</th>
			<th width="100px">FAQ</th>
			<th width="500px">FAQA</th>
	...
	<div style="max-height: 200px; overflow: scroll;">
		<table style="overflow-y: scroll; text-align: center;">
			...
			<tr>
				<td width="100px"><input type="text" class="faqId" value="<?=$row->faqId;?>" disabled="disabled"/></td>
				<td width="100px"><input type="text" class="faq" value="<?=$row->faq;?>"/></td>
				<td width="200px"><textarea rows="3" class="faqa" cols="30"><?=$row->faqa;?></textarea></td>
...
<div>
	<br/>
	Insert new row here
	<form name="form1" method="POST" action="<?php echo base_url();?>v1_0/insert/faqs">
		<table>
			<tr>
				<td>FAQ</td>
				<td><textarea name="faq" rows="5" cols="50"></textarea></td>
			</tr>
			<tr>
				<td>FAQA</td>
				<td><input name="faqa" type="text"/></td>
			</tr>
			<tr>
				<td>VIEW STATUS</td>
