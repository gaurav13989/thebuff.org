...
<script type="text/javascript">
	$(function(){
		$('.edit').live('click',function(){
	        ...
	        p['description'] = $(this).parent().parent().find('.desc').val();
	        ...
	        $(this).parent().load('/v1_0/update/careers/'+p['id'],p,function(html){
	        	if(html == "y")
		        {
		...
		$('.delete').live("click",function(){
			...
			if(temp)
			{
		        ...
		        p['id'] = $(this).parent().parent().find('.id').val();
		        p['title'] = $(this).parent().parent().find('.title').val();
		        p['description'] = $(this).parent().parent().find('.desc').html();
		        p['contact'] = $(this).parent().parent().find('.contact').val();
		        p['viewStatus'] = $(this).parent().parent().find('.viewStatus').val();;
		        
		        $(this).parent().parent().find(".loading").html('<img src="<?=base_url();?>images/loading.gif" />');
		        $(this).parent().load('/v1_0/delete/careers/'+p['id'],p,function(html){
		        	if(html == "y")
		        	{
		        		$(this).parent().find(".loading").html('<img src="<?=base_url();?>images/tick.jpg" />');
	...
	<div style="max-height: 200px; overflow: scroll;">
		<table style="overflow-y: scroll; text-align: center;">
			...
			<tr>
				...
				<td width="200px"><textarea rows="3" class="desc" cols="30"><?=$row->description;?></textarea></td>
...
<div>
	<br/>
	Insert new row here
	<form name="form1" method="POST" action="<?php echo base_url();?>v1_0/insert/careers">
		<table>
			<tr>
				<td>TITLE</td>
				<td><input name="title" maxLength="100" type="text"/></td>
			</tr>
			<tr>
				<td>DESCRIPTION</td>
